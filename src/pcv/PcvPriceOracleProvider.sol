...
interface NewUniSwapPriceOracle {
...
contract pcvPriceProvider is Ownable{
    ...
    function _getUnderlyingPriceInternal(address _pToken, TokenConfig memory tokenConfig, PriceOracle memory priceOracle) internal view returns (uint){
        ...
        if (sourceType == PriceOracleType.ChainLink) {
            ...
            return _getSwapPriceInternal(priceOracle, tokenConfig);
        } else if (sourceType == PriceOracleType.LP) {
            return _getLpPriceInternal(priceOracle, tokenConfig);
    ...
    function _getSwapPriceInternal(PriceOracle memory priceOracle, TokenConfig memory tokenConfig) internal view returns (uint) {
        address source = priceOracle.source;
        NewUniSwapPriceOracle swapPriceOracle = NewUniSwapPriceOracle(source);
        uint price = swapPriceOracle.getSwapTokenPrice(tokenConfig.underlying);
        if (price <= 0) {
            return 0;
        } else {//return: (price / 1e8) * (1e36 / baseUnit) ==> price * 1e28 / baseUnit
            return uint(price).mul(1e18).div(tokenConfig.baseUnit);
        }
    }

    function _getLpPriceInternal(PriceOracle memory priceOracle, TokenConfig memory tokenConfig) internal view returns (uint) {
        address source = priceOracle.source;
        NewUniSwapPriceOracle lpPriceOracle = NewUniSwapPriceOracle(source);
        uint price = lpPriceOracle.getSwapLpPrice(tokenConfig.underlying);
        return price;
    }
